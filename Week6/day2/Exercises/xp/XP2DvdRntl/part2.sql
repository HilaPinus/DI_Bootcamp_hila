...
-- Write a query to get the film ID, title, description, year of release and rental rate in ascending order
...
SELECT film_id, title, description, length, rental_rate FROM film WHERE title LIKE ('Chamber Italian');
SELECT film_id, title, description, length, rental_rate FROM film WHERE title IN ('Chamber Italian');
...
SELECT film_id, title, description, le	ngth, rental_rate FROM film WHERE title LIKE ('Cha%');
...
SELECT title, replacement_cost FROM film ORDER BY replacement_cost DESC OFFSET 10 LIMIT 10;

-- Write a query which will join the data in the customer table and the payment table. 
--You want to get the first name and last name from the curstomer table, as well as the amount and the 
--date of every payment made by a customer, ordered by their id (from 1 to…).
...
SELECT DISTINCT CONCAT(city, ' IS IN ', country) 
FROM city 
INNER JOIN country
ON city.country_id = country.country_id

-- Bonus You want to be able to see how your sellers have been doing? 
--Write a query to get the customer’s id, names (first and last), the amount and the date of payment ordered 
